void  invioPWM() {
  if (myPort!=null) {
    ...
    PWM_value.setText(String.format("%d", valorePWM));
  }
}

void start_PWM() {
  set_invio_PWM=PWM_value_slider.getValueI();
  tenpo_invio_PWM=PWM_time_slider.getValueI();
  if (myPort!=null) {
...
void smagnete_PWM() {
  ...
  if ( stato_s_magnete==1) {
    ...
    if (!timer1.isRunning()) {
      ...
      int decremento_Smagnete=PWM_value_slider.getValueI();
      ...
      PWM_value_slider.setValue(nuovo_slider_PWM);
      start_PWM();
      if (valorePWM==0) {
        stato_s_magnete=0;
      }
